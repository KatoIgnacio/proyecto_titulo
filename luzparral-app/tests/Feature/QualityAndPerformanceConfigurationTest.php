...
class QualityAndPerformanceConfigurationTest extends TestCase
{
    public function test_performance_script_covers_the_expected_load_and_routes_without_embedded_secrets(): void
    {
        ...
        $this->assertStringContainsString("'/dashboard?range=all'", $contents);
        $this->assertStringContainsString("'/contingencias/mapa?range=all&status=active'", $contents);
        ...
        $this->assertStringContainsString("'/informes?range=all'", $contents);
        $this->assertStringContainsString("'/informes/contingencias.csv?range=all'", $contents);
        $this->assertStringContainsString("'/informes/contingencias.pdf?range=all&report_type=executive'", $contents);
